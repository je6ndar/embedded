...
#define BCM2836_GPIO_ADDRESS 0x3F200000
...
ssize_t lll_write(struct file *file, const char __user *user, size_t size, loff_t *off)
{
	...
	if (pin > 21 || pin < 0)
	{
		printk("Invalid pin numver submitted\n");
		return size;
	}
	if (value != 0 && value != 1)
	{
...
//When user reads the file we call read func
//When user writes to the file we call write func
...
static int __init gpio_driver_init(void)
{
	...
	gpio_registers = (int*)ioremap(BCM2836_GPIO_ADDRESS, PAGE_SIZE);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("je6ndar");
MODULE_DESCRIPTION("LED Blink");
MODULE_VERSION("1.0");
